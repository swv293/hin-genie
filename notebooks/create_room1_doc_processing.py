...
# MAGIC %md
# MAGIC # Room 1 — Clinical Document Processing Intelligence
# MAGIC
# MAGIC Creates (or refreshes) the Genie Room over the document processing pipeline. Idempotent: the notebook
# MAGIC detects an existing room with the same display name and skips re-creation, so it can be safely re-run
# MAGIC after edits to the curation JSON.
# MAGIC
# MAGIC **What this notebook does**
# MAGIC 1. Reads the full curation (tables, sample questions, instructions, sample SQLs, joins, snippets,
# MAGIC    benchmarks) from `genie_config/room1_curation.json`.
...
# MAGIC - The `genie_config/room1_curation.json` file from this repo is uploaded to the workspace at the path
...
dbutils.widgets.text(
    "curation_json_path",
    "/Workspace/Repos/swami.venkatesh@databricks.com/hin-genie/genie_config/room1_curation.json",
    "Path to room1_curation.json",
)
...
# Read back the live state by issuing a no-op PATCH (same description) — the response includes serialized_space.
...
print(f"Try a question: 'How many documents came in yesterday?' or 'Show me the top 10 highest-risk documents'")
